...
class ProdDbManager(context: Context) {
    ...
    fun readProduct(searchText : String): ArrayList<ProdItem> {
        val dataList = ArrayList<ProdItem>()
        val selection = "${ProdDbNameClass.COLUMN_NAME_TITLE} like ?"
        val cursor = db?.query(ProdDbNameClass.TABLE_NAME, null, selection, arrayOf("$searchText"), null, null, null)
        if (cursor != null) {
            while (cursor.moveToNext()) {
                val dataTitle =
                    cursor.getString(cursor.getColumnIndexOrThrow(ProdDbNameClass.COLUMN_NAME_TITLE))
    ...
    fun readIngred(id: Int): ArrayList<ProdItem> {
        val dataList = ArrayList<ProdItem>()
        val selection = "_id like ?"
        val cursor = db?.query(ProdDbNameClass.TABLE_NAME, null, selection, arrayOf(id.toString()), null, null, null)
        if (cursor != null) {
            while (cursor.moveToNext()) {
                val dataTitle =
                    cursor.getString(cursor.getColumnIndexOrThrow(ProdDbNameClass.COLUMN_NAME_TITLE))
